...
namespace Bruneability_Portal.Controllers
{
    public class AdminController : BaseController
    {
        ...
        public ActionResult ConversionTiers()
        {
            return View();
        }
        ...
        public ActionResult AddNewMake()
        {
            try
            {
                return PartialView("_AddNewMake", new AddNewMake());
            }
            catch (Exception ex)
            {
                return Json(new { status = false, message = ex.Message }, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult GetMakesDataList()
        {
            string jsondata = string.Empty;
            GetMakesDataList GetMakes = new GetMakesDataList();
            try
            {
                HeaderToken request = new HeaderToken();
                string url = utilityRespository.getBaseUrl() + "Admin/GetMakesList";
                string strResponse = HttpApi.CreateRequest(url, request);
                GetMakes = JsonConvert.DeserializeObject<GetMakesDataList>(strResponse);


                jsondata = JsonConvert.SerializeObject(GetMakes, Formatting.Indented);
                return Json(new { jsondata }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                GetMakes.status = false;
                GetMakes.message = ex.Message;
                jsondata = JsonConvert.SerializeObject(GetMakes, Formatting.Indented);
                return Json(new { jsondata }, JsonRequestBehavior.AllowGet);
            }
        }
        public ActionResult GetMakesByID(int ID)
        {
            GetMakesForm makesdata = new GetMakesForm();
            try
            {
                GetMAKEbyID make = new GetMAKEbyID();
                make.Id = Convert.ToInt32(ID);
                string url = utilityRespository.getBaseUrl() + "Admin/GetMakesByID";
                string strResponse = HttpApi.CreateRequest(url, make);
                makesdata = JsonConvert.DeserializeObject<GetMakesForm>(strResponse);

                //employeesData.EmpData.JoiningDate = null;
                if (makesdata.MakeData == null)
                {
                    makesdata.MakeData = new AddNewMake();
                    makesdata.MakeData.Status = makesdata.status;
                    makesdata.MakeData.Message = makesdata.message;
                }
                else
                {
                    makesdata.MakeData.Status = makesdata.status;
                ...
                return PartialView("_AddNewMake", makesdata.MakeData);
            }
            catch (Exception ex)
            {
                makesdata.MakeData = new AddNewMake();
                makesdata.MakeData.Status = false;
                makesdata.MakeData.Message = ex.Message;
                return PartialView("_AddNewMake", makesdata.MakeData);
            }
        ...
        [HttpPost]
        public ActionResult AddMake(string make)
        {
            try
            {
                if (make != null)
                {
                    AddNewMake makeMO = new AddNewMake();
                    makeMO.Makes = make;
                    string url = utilityRespository.getBaseUrl() + "Admin/AddNewMake";
                    string strResponse = HttpApi.CreateRequest(url, makeMO);
        ...
        public ActionResult EditMake(int makeid, string make)
        {
            try
            {
                if (make != null)
                {
                    AddNewMake makeMO = new AddNewMake();
                    makeMO.Makes = make;
                    makeMO.makeID = makeid;
                    string url = utilityRespository.getBaseUrl() + "Admin/EditMakes";
                    string strResponse = HttpApi.CreateRequest(url, makeMO);
                    HeaderResponse Response = JsonConvert.DeserializeObject<HeaderResponse>(strResponse);
                    return Json(new { status = Response.status, message = Response.message }, JsonRequestBehavior.AllowGet);
                }
        ...
        public ActionResult DeleteMakeByID(int ID)
        {
            try
            {
                if (ID != 0)
                {
                    GetMAKEbyID make = new GetMAKEbyID();
                    make.Id = Convert.ToInt32(ID);
                    string url = utilityRespository.getBaseUrl() + "Admin/DeleteMakeByID";
                    string strResponse = HttpApi.CreateRequest(url, make);
                    HeaderResponse Response = JsonConvert.DeserializeObject<HeaderResponse>(strResponse);
                ...
                {
                    return Json(new { status = false, message = "Make ID is Null." }, JsonRequestBehavior.AllowGet);
                }
        ...
        public ActionResult UpdateTierAmount(TiersAmount model)
        {
            try
            {

                string url = utilityRespository.getBaseUrl() + "Admin/UpdateTiersDetails";
                string strResponse = HttpApi.CreateRequest(url, model);
            ...
            {
                return Json(new { Modelstate = ModelState, status = false, message = "" }, JsonRequestBehavior.AllowGet);
            }
        }
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (emp != null)
                    {
                        ...
                        emp.UserId = Convert.ToInt32(CurrentUser.SessionUser.ID);
        ...
        public ActionResult GetConversionByID(int ID)
        {
            ...
            conversionData.ConversionData.TierList = getTierList();
            conversionData.ConversionData.MakeList = getMakeList();
            try
            {
                //vt_UserProfile user = new vt_UserProfile();
        ...
        public SelectList RoleList()
        {
            ...
            string url = utilityRespository.getBaseUrl() + "Admin/RoleList";
            string strResponse = HttpApi.CreateRequest(url, request);
        ...
        public SelectList getTierList()
        {
            HeaderToken request = new HeaderToken();

            string url = utilityRespository.getBaseUrl() + "Admin/GetListofTiers";
            string strResponse = HttpApi.CreateRequest(url, request);
            ...
            newRow[1] = "Please Select";
            tester.Rows.InsertAt(newRow, 0);
            return new SelectList(tester.AsDataView(), "Id", "TierName");
        }
        public SelectList getMakeList()
        {
            HeaderToken request = new HeaderToken();

            string url = utilityRespository.getBaseUrl() + "Admin/GetListofMakes";
            string strResponse = HttpApi.CreateRequest(url, request);

            DataTable tester = (DataTable)JsonConvert.DeserializeObject(strResponse, (typeof(DataTable)));
            return new SelectList(tester.AsDataView(), "Id", "Makes");
        }
        ...
        public JsonResult GetTiersDetails()
        {
            try
            {
                string url = utilityRespository.getBaseUrl() + "Admin/GetTierDetails";
                string strResponse = HttpApi.CreateRequest(url);
                GetTiersDetails markupData = JsonConvert.DeserializeObject<GetTiersDetails>(strResponse);
